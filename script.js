...
// ============================================================
// 6. DYNAMIC PERSONALIZATION (SINGLE = WALIMA ONLY, FAMILY = ALL)
// ============================================================
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (guestParam) {
    globalCleanGuestName = decodeURIComponent(guestParam.replace(/\+/g, ' ')).trim();

    // 1. Envelope ke bottom par personalized naam
    ...
    // 2. Scratch card me dynamic guest name
    const scratchSub = document.querySelector('.hidden-p');
    if (scratchSub) {
      scratchSub.innerText = `Specially Invited: ${globalCleanGuestName}`;
    }

    // 3. RSVP form me name auto-fill
    const nameInput = document.getElementById('guestName');
    if (nameInput) {
      nameInput.value = globalCleanGuestName;
    }

    // 4. SMART CHECK: Family vs Single Guest
    const isFamily = globalCleanGuestName.toLowerCase().includes('family');
