...
const HOST_WHATSAPP_1 = "923126611363"; 

// Doosra WhatsApp Number
const HOST_WHATSAPP_2 = "923370723980"; 
...
// Wedding Event Date (Countdown Target: 16 Oct 2026, 07:00 PM Mehndi)
const TARGET_DATE = new Date('October 16, 2026 19:00:00').getTime();
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (rawSearch.includes('guest=')) {
    ...
    // Form elements & event elements
    const countWrapper = document.getElementById('guestCountWrapper');
    const guestCountSelect = document.getElementById('guestCount');

    const eventMehndi = document.getElementById('eventMehndi');
    const eventDeparture = document.getElementById('eventDeparture');
    ...
    const scratchDatePill = document.getElementById('scratchDatePill');
    const footerDate = document.getElementById('footerDate');

    if (!isFamily) {
      // --- SINGLE GUEST (SIRF WALIMA INVITE - 18 OCT ONLY) ---
      isSingleGuestMode = true;

      // 1. Total Persons hide karein aur 1 Person par lock karein
      ...
      // 2. Mehndi, Departure, aur Barat teeno ko hide karein
      if (eventMehndi) eventMehndi.style.display = 'none';
      if (eventDeparture) eventDeparture.style.display = 'none';
      if (eventBarat) eventBarat.style.display = 'none';

      // 3. Walima card ko center align aur highlight karein
      if (timelineGrid) {
        timelineGrid.style.display = 'flex';
        timelineGrid.style.justifyContent = 'center';
      }
      if (eventWalima) {
        eventWalima.style.maxWidth = '440px';
        eventWalima.style.width = '100%';
        eventWalima.classList.add('highlight-box');
      }

      // 4. Poori website par date sirf 18th October Walima show karein
      if (envelopeDate) envelopeDate.innerText = "18 OCTOBER 2026";
      if (heroDateBadge) heroDateBadge.innerText = "OCTOBER 18, 2026 (WALIMA)";
      if (scratchDatePill) scratchDatePill.innerText = "18th October 2026";
      if (footerDate) footerDate.innerText = "18th October 2026 • Sittara Marriage Hall, Bhakkar";

    } else {
      // --- FAMILY GUEST (MEHNDI + BARAT + WALIMA ALL EVENTS) ---
      isSingleGuestMode = false;
      if (eventMehndi) eventMehndi.style.display = 'block';
      if (eventDeparture) eventDeparture.style.display = 'block';
      if (eventBarat) eventBarat.style.display = 'block';

      if (envelopeDate) envelopeDate.innerText = "16 - 18 OCTOBER 2026";
      if (heroDateBadge) heroDateBadge.innerText = "OCTOBER 16 - 18, 2026";
      if (scratchDatePill) scratchDatePill.innerText = "16th - 18th October 2026";
      if (footerDate) footerDate.innerText = "16th – 18th October 2026 • Sittara Marriage Hall, Bhakkar";
    }
  }
});
...
const urduTranslations = {
  ...
  weddingBadge: "۱۶ - ۱۸ اکتوبر ۲۰۲۶",
  ...
  mehndiTitle: "رسمِ حنا (مہندی)",
  mehndiDate: "جمعہ، ۱۶ اکتوبر ۲۰۲۶",
  mehndiDesc: "روایتی مہندی کی تقریب، رنگ اور پرمسرت خاندانی خوشیاں۔",
  departureTitle: "روانگی بارات",
  departureDate: "ہفتہ، ۱۷ اکتوبر ۲۰۲۶",
  departureDesc: "بارات کی باوقار روانگی کے لیے معزز مہمانوں کا اجتماع۔",
...
const englishOriginals = {
  ...
  weddingBadge: "OCTOBER 16 - 18, 2026",
  ...
  mehndiTitle: "Mehndi Celebration",
  mehndiDate: "Friday, 16th October 2026",
  mehndiDesc: "An evening of traditional henna rituals, vibrant colors, and joyous family celebrations.",
  departureTitle: "Departure of Barat",
  departureDate: "Saturday, 17th October 2026",
  departureDesc: "Gathering of family and guests for the grand departure of Barat procession.",
...
function applyLanguage(lang) {
  ...
  if (eventMehndi) {
    eventMehndi.querySelector('h3').innerText = t.mehndiTitle;
    eventMehndi.querySelector('.event-day').innerText = t.mehndiDate;
    eventMehndi.querySelector('.event-text').innerText = t.mehndiDesc;
  }

  if (eventDeparture) {
    eventDeparture.querySelector('h3').innerText = t.departureTitle;
    eventDeparture.querySelector('.event-day').innerText = t.departureDate;
    eventDeparture.querySelector('.event-text').innerText = t.departureDesc;
  }

  if (eventBarat) {
    eventBarat.querySelector('h3').innerText = t.baratTitle;
    eventBarat.querySelector('.event-day').innerText = t.departureDate;
